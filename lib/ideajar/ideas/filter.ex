defmodule Ideajar.Ideas.Filter do
  ...
  This commit is a **conservative extraction**: the 3 existing clauses
  (required, optional, durations) move verbatim — same subquery shapes,
  same `Enum.uniq` normalization, same NULL-exclusion semantics — so the
  slice-4 SQL-emission pins (`HAVING COUNT(DISTINCT …)`) and the slice-5
  NULL-exclusion contract (AA7) hold without test changes. Slice 6 step 6
  extends this module with `apply_max_cost/2` (rule of 4 fires, see BB4).
  ...
  Each clause defaults to an empty list (= inactive). Empty lists are
  no-ops — the input query is returned unchanged at the SQL layer.
