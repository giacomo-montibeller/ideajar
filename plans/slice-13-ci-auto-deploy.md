...
### Documentation
- [x] **D1** — `docs/deploy.md` §Routine deploys riscritto: apre con una frase che dichiara "Sotto condizioni normali nessuna azione manuale è necessaria; se `deploy.yml` segnala uno smoke-test failure il release precedente sta ancora servendo — vedi §Common failure modes". Sotto-sezioni "Automatic (default)" + "Manual fallback" che preservano il comando `git push gigalixir main`. Preambolo del file aggiornato in coerenza, con regression pin.
